...
bool PHAL_FLASH_erase(uint32_t address, uint32_t length_bytes) {
    ...
    uint32_t first_page       = (address - FLASH_BASE) / page_size_bytes;
    uint32_t last_address     = address + length_bytes - 1U;
    uint32_t last_page        = (last_address - FLASH_BASE) / page_size_bytes;
    uint32_t page_address     = FLASH_BASE + first_page * page_size_bytes;
    uint32_t last_page_address = FLASH_BASE + last_page * page_size_bytes;
    ...
    while (success) {
        ...
        if (page_address == last_page_address) {
            break;
        }
